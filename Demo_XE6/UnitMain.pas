...
uses
  ...
  FMX.StdCtrls, FMX.Viewport3D, FMX.Forms3D, FMXTee.Canvas, FMXTee.Chart;
...
var tmp : TFormAbout;
begin
  tmp:=TFormAbout.Create(Self);

  {$IFDEF ANDROID}
  tmp.ShowModal(procedure(ModalResult: TModalResult)
        begin
          tmp.DisposeOf;
        end);
  {$ELSE}
...
begin
  if Assigned(TreeView1.Selected) and
     TreeView1.Selected.Enabled and (TreeView1.Selected.StyleName<>'') then
  begin
